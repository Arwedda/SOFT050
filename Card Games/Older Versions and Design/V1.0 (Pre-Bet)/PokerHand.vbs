    ...
    Sub DistributeChips(StartChips)
        iPChips = StartChips
        iC1Chips = StartChips
    End Sub
    ...
    Sub DrawCards()
        If iRound = 0 Then
            ...
            Do While sPCard(0) = sPCard(1) or sPCard(0) = sC1Card(0) or sPcard(1) = sC1Card(0) or sC1Card(0) = sC1Card(1)
                PickCards sPCard, 0, 1
                PickCards SC1Card, 0, 1
            Loop
        ElseIf iRound = 1 Then
            PickCards sCCard, 0, 2
            Do While sCCard(0) = sPCard(0) or sCCard(0) = sPCard(1) or sCCard(0) = sC1Card(0) or sCCard(0) = sC1Card(1) or sCCard(0) = sCCard(1) or sCCard(0) = sCCard(2) _
            or sCCard(1) = sPCard(0) or sCCard(1) = sPCard(1) or sCCard(1) = sC1Card(0) or sCCard(1) = sC1Card(1)  or sCCard(1) = sCCard(2) _
            or sCCard(2) = sPCard(0) or sCCard(2) = sPCard(1) or sCCard(2) = sC1Card(0) or sCCard(2) = sC1Card(1)
                PickCards sCCard, 0, 2
            Loop
        ElseIf iRound = 2 Then
            PickCards sCCard, 3, 3
            Do While sCCard(3) = sPCard(0) or sCCard(3) = sPCard(1) or sCCard(3) = sCCard(0) or sCCard(3) = sCCard(1) or sCCard(3) = sCCard(2) _
            or sCCard(3) = sC1Card(0) or sCCard(3) = sC1Card(1)
                PickCards sCCard, 3, 3
            Loop
        ElseIf iRound = 3 Then
            PickCards sCCard, 4, 4
            Do While sCCard(4) = sPCard(0) or sCCard(4) = sPCard(1) or sCCard(4) = sCCard(0) or sCCard(4) = sCCard(1) or sCCard(4) = sCCard(2) or sCCard(4) = sCCard(3) _
            or sCCard(4) = sC1Card(0) or sCCard(4) = sC1Card(1)
                PickCards sCCard, 4, 4
            Loop
        End If
    End Sub
    ...
    Sub SuitsAndValues(CV, CS, pic, Cards, j, k)
        Dim i
        For i = j to k
            CS(i) = Left(sDeck(Cards(i)), 1)
            CV(i) = CINT(Mid(sDeck(Cards(i)), 2, 2))
        Next
    End Sub
    ...
    Sub ShowCards(pic, Cards, j, k)
        Dim i
        For i = j to k
            document.getElementById(pic & i).src = sDeck(Cards(i))
        Next
    End Sub
    ...
    Sub ShowWinner()
        If iPBestScore > iC1BestScore Then
            sHandRes = "You win with"
            sPBestHandDesc = sHandRes & Mid(sPBestHandDesc, 9)
            ShowBest sPBestHandDesc, parResult, iPBestScore
        Else
            sHandRes = "Computer 1 wins with"
            sC1BestHandDesc = sHandRes & Mid(sC1BestHandDesc, 15)
            ShowBest sC1BestHandDesc, parResult, iC1BestScore
        End If
    End Sub
    
    Sub ShowBest(Desc, Par, Score)
        Par.innerText = Desc & " " & Score
    End Sub